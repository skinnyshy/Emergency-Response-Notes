...
![](http://img-upaiyun-own.test.upcdn.net/win-5-1.png)
...
![](http://img-upaiyun-own.test.upcdn.net/win-5-4.png)

另外，日志并未记录暴力破解的IP地址，我们可以使用Wireshark对捕获到的流量进行分析，获取到正在进行爆破的IP：

![](http://img-upaiyun-own.test.upcdn.net/win-5-5.png)

通过对近段时间的管理员登录日志进行分析，如下：

![](http://img-upaiyun-own.test.upcdn.net/win-5-6.png)

管理员登录正常，并未发现异常登录时间和异常登录ip，这里的登录类型10，代表远程管理桌面登录。

另外，通过查看FTP站点，发现只有一个测试文件，与站点目录并不在同一个目录下面，进一步验证了FTP暴力破解并未成功。

![](http://img-upaiyun-own.test.upcdn.net/win-5-7.png)
